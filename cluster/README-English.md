...
In real test for large scale of GBytes level of files, single node setting with 25 or more threads (5 Files X 5 Threads) can reach 1Gbps traffic between Global and China Amazon S3. If they are most MBytes level files, you can setup more concurrency for one Amazon EC2 node.  
